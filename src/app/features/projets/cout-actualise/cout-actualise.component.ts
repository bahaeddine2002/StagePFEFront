...
import { CoutActualiseService } from "../services/cout-actualise.service";
import {
...
@Component({
  selector: "app-cout-actualise",
  templateUrl: "./cout-actualise.component.html",
  styleUrls: ["./cout-actualise.component.css"],
})
export class CoutActualiseComponent implements OnInit {
  ...
  constructor(
    ...
    private coutActualiseService: CoutActualiseService,
  ...
  ngOnInit(): void {
    ...
    this.loadCoutActualise();
  }

  loadCoutActualise(): void {
    this.isLoading = true;
    this.errorMessage = null;

    this.coutActualiseService
      .getCoutActualise(this.projectId)
      ...
      .subscribe({
        ...
        error: (error) => {
          this.errorMessage = this.extractErrorMessage(
            error,
            "Impossible de charger le coût actualisé.",
          );
  ...
  get totalJoursActualisesGlobal(): number {
  ...
  get totalCoutActualiseGlobal(): number {
    return this.resources.reduce((total, resource) => {
      return (
  ...
  get budgetConsommePercent(): number {
    // TODO: Replace this later with the real project budget from backend.
    ...
    return (this.totalCoutActualiseGlobal / budgetProjet) * 100;
  }
  ...
  save(): void {
    ...
    this.coutActualiseService
      .saveCoutActualise(this.projectId, payload)
      .pipe(finalize(() => (this.isSaving = false)))
      .subscribe({
        next: () => {
          this.toastService.success("Coût actualisé enregistré avec succès.");
          this.isDirty = false;
          this.loadCoutActualise();
        },
        error: (error) => {
          const message = this.extractErrorMessage(
            error,
            "Erreur lors de l’enregistrement du coût actualisé.",
          );
  ...
  getCumulDaysRemaining(monthIndex: number): number {
    const total = this.totalJoursActualisesAnnee;
  ...
  getCumulCostRemaining(monthIndex: number): number {
    const total = this.totalCoutActualiseAnnee;
